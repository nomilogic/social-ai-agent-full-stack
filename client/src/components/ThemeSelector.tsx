...
export const ThemeSelector: React.FC = () => {
  ...
  return (
    <div className="relative group">
      <button className="flex items-center space-x-2 theme-button-secondary px-4 py-2 rounded-lg hover:theme-bg-card transition-colors">
        <Palette className="w-5 h-5 theme-text-primary" />
        <span className="theme-text-primary font-medium">{currentTheme.name}</span>
      </button>
      ...
      <div className="absolute top-full right-0 mt-2 w-72 theme-bg-card rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
        <div className="p-4">
          <h3 className="theme-text-primary font-semibold mb-3">Choose App Theme</h3>
            ...
            {availableThemes.map((theme) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className={`w-4 h-4 rounded-full bg-gradient-to-r ${theme.bgGradient}`}></div>
                  <span className="theme-text-primary font-medium">{theme.name}</span>
                </div>
          ...
          <div className="mt-3 p-2 theme-bg-primary rounded-lg">
            <p className="text-xs theme-text-light">
              Themes are based on the onboarding carousel color schemes. Changes apply instantly across the entire app.
            </p>
          </div>
        </div>
      </div>
